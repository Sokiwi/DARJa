...
get_ARIs <- function() {
  ...
  write.table(results, file="optimal_number_clusters3.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
}
# runs as get_ARIs()
...
sil_res <- data.frame(1:101, all_vals[[1]], all_vals[[2]], 
      all_vals[[3]], all_vals[[4]], all_vals[[5]])
names(sil_res) <- c("k", "WardD", "WardD2", "complete", "UPGMA", "WPGMA")
write.table(sil_res, file="optimal_number_clusters_sil2.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
methods <- c("ward.D", "ward.D2", "complete", "average", "mcquitty")
results <- list()
for (i in 1:5) {
  results[i][1] <- NA
  for (K in 2:10) {
    cat("doing method", i, "for k =", K, "\n")
    cb <- clusterboot(m, B = 100, bootmethod="boot", clustermethod=disthclustCBI, 
                      method=methods[i], k = K, showplots=FALSE)
    res <- round(mean(cb$bootmean),4)
    results[[i]][K] <- res
  }
}
out <- data.frame(1:10, results[[1]], results[[2]], results[[3]], results[[4]], results[[5]])
out <- out[-1,]
