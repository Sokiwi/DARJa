...
# various errors contained in the Excel files are now corrected
# feature_values_changes.txt is a guide to the corrections
cg <- read.table(file="feature_values_changes.txt", sep="\t", header=TRUE)  # corrections guide
# data points to delete are those where the "To" column in cg has a zero
fv_out <- cg$From[which(cg$To==0)]
out <- c()
count <- 0
for (i in 1:nrow(d)) {
  if (d$fv[i] %in% fv_out) {
...
d <- d[-out,]
# now deal with features and feature values whose names should be changed
dealt.with <- which(cg$To==0)
cg2 <- cg[-dealt.with,]
bados <- cg2$From
count <- 0
sanity.check <- c()
for (i in 1:nrow(d)) {
  if (d$fv[i] %in% bados) {
    count <- count + 1
    sanity.check[count] <- i
    w_bados <- which(bados==d$fv[i])
    new_fv <- cg2$To[w_bados]
    new_f <- paste(strsplit(new_fv, "_")[[1]][1:2], collapse="_")
    d$f[i] <- new_f
    d$fv[i] <- new_fv
  }
}

save(d, file="darja_data4.RData")
write.table(d, file="darja_data4.txt", sep="\t", quote=FALSE)
